...
OUTPUT_DIR   = "/home/xzh5180/Research/vlm-mobility/outputs/usecase1_finetune/Qwen2.5-VL-7B-full/"
...
def prepare_splits(data_dir, annotations):
    ...
    val_list   = pool[:N_VAL]
    train_list = pool[N_VAL:]  # use all available training data
...
def build_inputs_with_labels(processor, image_path, gt_json, device):
    ...
    full_enc   = processor(text=[full_text],   images=image_inputs, return_tensors="pt")
    ...
    if "pixel_values"   in full_enc:
        result["pixel_values"]   = full_enc["pixel_values"].to(device)
    if "image_grid_thw" in full_enc:
        result["image_grid_thw"] = full_enc["image_grid_thw"].to(device)
    return result
